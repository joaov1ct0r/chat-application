...
export default class SocketIO {
  ...
  private async get(channel: string) {
    const messages = await this._redis.get(channel)
    return messages
  }

  private async set(channel: string, data: IRedisData[]) {
  ...
  private async subscribe(socket: Socket) {
    await this._redis.subscribe(
      ...
      (error?: Error | null, result?: unknown) => {
        if (error) {
          throw new Error(`Erro ao obter mensagem - ${error}`)
        }

        const { user, message } = result as IRedisData
        socket.emit('messages', JSON.stringify({ user, message }))
      },
    )
  }
  ...
  public init(): void {
    console.log(
      'Socket running at ' + process.env.SOCKET_HOST + process.env.SOCKET_PORT,
    )
